...
export default function AnswerView(data: Data) {
  const [response, setResponse] = useState('');
  const hasRunRef = useRef(false); // substitute by status in data

  useEffect(() => {
    // if (!hasRunRef.current) {
    //   APIrequest(data);
    // }
    if (typeof data.messages.slice(-1)[0].content === 'string') {
      setResponse(data.messages.slice(-1)[0].content as string)
    } else if (typeof data.messages.slice(-1)[0].content == 'object') {
      // add non string logic
    }
  }, [data]);
